...
61:"Rain",63:"Heavy Rain",71:"Snow",95:"Thunderstorm"
};
...
async function load(name){
 ...
 try{
  ...
  const {latitude,longitude,name:city,country}=gd.results[0];

  const w=await fetch(`${WEA}?latitude=${latitude}&longitude=${longitude}&current_weather=true&timezone=auto`);
  const wd=await w.json();

  update(city,country,wd.current_weather,wd.timezone);
 }catch{
  desc.innerText="Not found";
 }
}
